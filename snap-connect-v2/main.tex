...
%
\mbox{Type} & \tau & ::= & \float_{\real \times \real}
\end{array}
\]
...
Let $\edistr_1 \in \distr(\float_{\real \times \real})$, $\edistr_2 \in \distr(\float_{\real \times \real})$ are defined:
\[
...
\[
	...
	\end{cases},
\]
where $\fvarx$ and $\rvary$ is the only random variable in $\edistr_1$ and $\edistr_2$ respectively. The $\rvarx_l, \rvarx_u$ and $\rvary_l, \rvary_u$ are deterministic functions of $\fvarx$ and $\fvary$ respectively in $\edistr_1$ and $\edistr_2$ . In our $\uniform$ distribution semantics, i.e., $\sem{\uniform(0, 1)}$, the deterministic function is equality.
\\
