...
\[
	u \xleftarrow{\$} \mu; y = \frac{\ln (u)}{\epsilon}; s \samplel \{-1, 1\}; z = s * y; x = f(a); w = x + z; w' = \lfloor w \rceil_{\Lambda}; r = \clamp_B (w')
\]
...
{
	w'_1 = \lfloor w_1 \rceil_{\Lambda} 
	\sim_{0, 0} w'_2 = \lfloor w_2 \rceil_{\Lambda} : w_1 + k = w_2 \land -2 \leq k \leq 0  \Rightarrow w'_1 + k = w'_2
}
\and
\inferrule*[right = AxNull]
{
...
\draw[very thick] (8, 3pt) -- (8, -3pt) node[anchor=north] {$\lfloor f(a) \rceil_{\Lambda}$};
...
\\
%
where $b$ is the greatest value rounded by $\Lambda$ that is smaller than $B$ and $t = \lfloor f(a) \rceil_{\Lambda} - f(a)$.
...
%%%%%%%%%%%%%%%%%%%Draw The Output Line%%%%%%%%%%%%%%%%%%
\draw[thick,->] (-1,0) -- (15,0) node[anchor=north west] {$\snap$};
%%%%%%%%%%%$$$%%%Draw Ticks on The Output Line%%%%%%%%%%%
\draw[very thick] (0 cm,3pt) -- (0 cm,-3pt) node[anchor=north] {-B};
\draw[very thick] (14 cm,3pt) -- (14 cm,-3pt) node[anchor=north] {B};
