...
\[
	u \xleftarrow{\$} \mu; y = \frac{\ln (|u|)}{\epsilon}; s \sign(u); z = s * y; x = f(a); w = x + z; w' = \lfloor w \rfloor_{\Lambda}; r = \clamp_B (w')
\]
...
{
	y_1 = \frac{\ln(|u_1|)}{\epsilon} \sim_{0, 0} y_2 = \frac{\ln(|u_2|)}{\epsilon} : e^{-\epsilon} u_2 \leq u_1 \leq e^{\epsilon} u_2  \Rightarrow y_2 - 1 \leq y_1 \leq 1 + y_2
}
\\\\
\inferrule
{
...
{
	s_1 = \sign(u_1) \sim_{0, 0} s_2 = \sign(u_2) : e^{-\epsilon} u_2 \leq u_1 \leq e^{\epsilon} u_2 \Rightarrow s_1 = s_2
}
\\\\
\inferrule
{
...
\[
	{\mu_2}(y) = {\uniform}(y)
\]
where $\uniform$ is uniform distribution over $(-1, 1)$ whoes $\pdf.$ is defined as:
\[
	\pdf_{\uniform}(x) = 
	\begin{cases}
	\frac{1}{2} & x \in (-1, 1)\\
...
\[
	{\mu_L}(x, y) = 
	\begin{cases}
	{\uniform}(x) & x \cdot e^{-\epsilon} = y \land x \in (-1, 1)\\
	0       & o.w.
	\end{cases}
	\\
	{\mu_R}(x, y) = 
	\begin{cases}
	{\uniform}(y) & x \cdot e^{-\epsilon} = y \land y \in (-1, 1)\\
	0       & o.w.
	\end{cases}.
\]
...
\[
	\pdf_{\mu_L}(x, y) = 
	\begin{cases}
	\pdf_{\uniform}(x) & x \cdot e^{-\epsilon} = y \land x \in (-1, 1)\\
	0       & o.w.
	\end{cases}
\]
\[
	\pdf_{\mu_R}(x, y) = 
	\begin{cases}
	\pdf_{\uniform}(y) & x \cdot e^{-\epsilon} = y \land y \in (-1, 1)\\
	0       & o.w.
	\end{cases}.
\]
\begin{itemize}
	...
	\begin{itemize}
		...
		\[
			\pdf_{\projl(\mu_L)}(x) = 
			\begin{cases}
			\int_{y}\pdf_{\uniform}(x) & (x,y) \in \Psi \land x \in (-1, 1)\\
			0       & o.w.
			\end{cases} 
			= 
			\begin{cases}
			\pdf_{\uniform}(x) & x \in (-1, 1)\\
			0       & o.w.
			\end{cases}
			=
			\pdf_{\mu_1}(x)
		...
		\[
			\pdf_{\projr(\mu_R)}(y) = 
			\begin{cases}
			\int_{x}\pdf_{\uniform}(y) & (x,y) \in \Psi \land y \in (-1, 1)\\
			0       & o.w.
			\end{cases} 
			= 
			\begin{cases}
			\pdf_{\uniform}(y) & y \in (-1, 1)\\
			0       & o.w.
			\end{cases}
			=
			\pdf_{\mu_2}(y)
	...
	By definition of $\epsilon-$DP divergence, we have:
	 ...
	 \]
	 \begin{itemize}
	 	\item[{\bf case}] $S \subseteq \{(x, y) | x \in (-1, 1) \land x \cdot e^{-\epsilon} = y\}$:\\
		 \[
		 \begin{array}{ll}
		 \Delta_{\epsilon}(\mu_L, \mu_R) 
		 & = 
		 ...
		 \end{array}
		 \]
	 	\item[{\bf case}] $S \subseteq \{(x, y) | x \in [1, e^{\epsilon}) \cup (-e^{\epsilon}, -1]  \land x \cdot e^{-\epsilon} = y\}$:\\
		 \[
		 \begin{array}{ll}
		 \Delta_{\epsilon}(\mu_L, \mu_R) 
		 & = 
