...
These computation errors can cause critical failure in preserving differential privacy. 

We develop methods for formally verifying differential privacy of algorithms implemented in floating-point computation. Our methods are based on an extension of the relational program logic $\mathsf{apRHL}^+$\cite{barthe2016proving} and an operational semantics over floating-point computation.

We demonstrate our methods on an algorithm: the \emph{Snapping mechanism}. This is an algorithm proposed in \cite{mironov2012significance}, where it is theoretically proved that even in floating-point computation it still preserves differential privacy.
...
The programs of differentially private algorithms can be formally verified via existing logics and tools\cite{barthe2016proving}. 
However, these verifications are under the assumption that the implementations of these algorithms are over real computing without computation error.
While in reality, these algorithms are implemented in floating-point computation with computation errors.
These computation errors can cause critical failure in preserving differential privacy, which is identified in several literatures \cite{balcer2017differential, gazeau2016preserving, mckenna2020permute, mironov2012significance, ilvento2020implementing}.
%
In the meantime, some algorithms are proposed claiming ability of preserving differential privacy even in floating-point computation, for instance:
\begin{itemize}
...
\end{itemize}
%
Unfortunately, existing pen-and-paper proofs of these algorithms are based on complex floating-point error approximation, which is hard to understand and error-prone.

This gives are the motivation that can we develop methods for formally and automatically verifying these algorithms by considering the error arose during the floating-point computation.
...
%
We prove the soundness of the logic via methodology of probabilistic coupling and approximate lifting shown by Barth et al in \cite{barthe2016proving}.
%
\section{Syntax}
The language is defined as follows.
\\
In the language, $\rval$ is in domain of real number $\real$, $\rval \in \real$ and $\fval$ is in domain of floating point number $\float$, $\fval \in \float$. 
Furthermore, the domain of floating point number $\float$ is a subset of $\real$ containing the real number that can be represented in the floating point computation. The type $\float_{\real \times \real}$ represents the base type of expressions. 
...
{
	\trsenv, \expr \trsto (\fval_1, (\ubar{\rval}, \bar{\rval}))
	~~~~
