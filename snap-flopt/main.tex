...
%
		...
		%
		Since $f(a) + 1 = f(a')$, we have $\round{f(a)} < \round{f(a')}$,
		$s = 1$ for input $a'$ and $s$ can be $1$ or $-1$ for input $a$.
		We have following subcases:
		%
		\subcaseL{$s = 1$ for input $a$}.
		%
		 ...
		 }
		 {
		  ...
		  }
		  {
		   \inferrule
		   {
			...
			(f(a) + \frac{1}{\epsilon}\frac{\oln(R)}{(1+\eta)^2}) / (1 + \eta)
			\bigg)
		   }
		   {
		...
		\end{mathpar}
		%
		Let $y_1 = x + \frac{\Lambda}{2}$, we have lower bound $\ubar{R}$ and upper bound $\bar{R}$ for $r$ be:
		%
		\[
		\bar{R} = e^{\epsilon
		}
