...
\\
1. any %
output of $\snap(a)$ in floating point computation is $\valv$,
%
\\
...
\clamp_B \big(
	...
	\big)
	 \rbigstep \valv.$$
%
We have $\Pr[\snap(a) = \valv] = \frac{1}{2} (\rvalR - \rvalL)$ 
...
%
%
$|\fvalR - \fvalL|$ and $|\fvalR' - \fvalL'|$
is bounded by $h(|{\rvalR} - {\rvalL}|)$, $g(|{\rvalR} - {\rvalL}|)$ and $h(|{\rvalR'} - {\rvalL'}|)$, $g(|{\rvalR'} - {\rvalL'}|)$,
%
s.t.:
%
\[
...
%
		\subcaseL{
		$\boldsymbol{\round{f(a)}_{\Lambda} > 0 \land \valv \in (0, \round{f(a)}_{\Lambda}) } $}
		%
