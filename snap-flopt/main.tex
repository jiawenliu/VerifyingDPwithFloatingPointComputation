...
%
Consider the $\snap(a)$ outputting the same result $x$ under floating point and real computaion, let $(L, R)$ be the range where $\forall u \in (L, R)$ and some $s$ s.t. $[U \mapsto u, S \mapsto s],\snap''(a) \rbigstep [z \mapsto \varx]$, we have $\Pr[\snap(a) = \varx] = R - L$. Sice the $\snap(a)$ is $\epsilon-$DP, we have:
\[
...
%
We estimate the worst case $|r - l|$ in terms of floating point relative error and $|R - L|$ through our semantics in order to verify the privacy loss of $\snap$.
	%
	\begin{itemize}
		%
		...
		\\
		%
		Let $L$ and $R$ be the range where $\forall u \in (L, R)$,  $[U \mapsto u, S \mapsto s],\snap''(a) \fbigstep [z \mapsto \varx]$ and
		...
		\\
		%
		We need to estimate the worst case range of $R$ in this case.
		The derivation of this case given $\trsenv = [U \mapsto (R, (R, R)), S \mapsto (1, (1, 1))]$ is shown as following:
		% \begin{figure}
		\begin{mathpar}
		\inferrule[uop]
		{
...
%
		...
		%
		In the same way, we have the derivation for $\snap'(a', l, 1)$ and $\snap'(a', r, 1)$:
		\begin{mathpar}
		\inferrule
		{
			\dots
