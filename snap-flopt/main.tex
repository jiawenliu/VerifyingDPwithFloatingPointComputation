...
%
\begin{figure}
\boxed{\trsenv, \expr \trsto \fval * \rval * \rval}
\begin{mathpar}
\inferrule*[right = var]
{
	\trsenv(\varx) 
...
\inferrule*[right = sample]
{
	 ...
	 % \and
	 % \rval^1 \leq \fval \leq \rval^2
	 ~~~~
	 \rval^{l} \leq \fval \leq \rval^{u}
}
{
	\trsenv, \varx \samplel \edistr
	\trsto
	\trsenv[\varx \mapsto (\fval, (\rval^{l}, \rval^{u}))]
}
\end{mathpar}
...
%
	...
	\inferrule*[right = sample]
	{
		 \fval \leftarrow \edistr^{\diamond}
		 \and
	 	\rval^1 \leq \fval \leq \rval^2 ~ (\square)
	}
	{
		\trsenv, \varx \samplel \edistr
		\trsto
	...
	%
	Since we know $\trsenv$ is a bounded environment and $\rval^1 \leq \fval \leq \rval^2$ by assumption $(\star)$ and $(\square)$ .
	%
	\\
	%
...
%
	...
	\\
	%
	By (1), (2) and hypothesis: $\fval^1 < 0$, $\fval^2 \geq 0$ and $ * \in \{\times, \div\}$, we have:
	%
	$\fval < 0
	\land
	\ubar{\rval'}
	...
	%
	Let 
	$\bar{\rval'} = \bar{\rval^1} * \bar{\rval^2}$
	and 
	%
	$\ubar{\rval'} = \ubar{\rval^1} * \ubar{\rval^2}$.
	%
	...
	%
	Let 
	$\bar{\rval'} = \ubar{\rval^1} * \ubar{\rval^2}$
	and 
	%
	$\ubar{\rval'} = \bar{\rval^1} * \bar{\rval^2}$.
	%
...
% $\expr_{\snap''} = \clamp_B \big( \round{\rval^{\snap'} }_{\Lambda} \big)$
% %
% \\
% %
...
%
	...
	\end{itemize}
		...
		%
		%
	\caseL{$\boldsymbol{\valv \in (\round{f(a)}_{\Lambda}, \round{f(a')}_{\Lambda})}$}
		%
...
%
		...
		%
		This case can also be split into 3 subcases symmetric with the case where $\boldsymbol{\valv \in  (-B, \round{f(a)}_{\Lambda})}$:
		%
		%
		%
		\subcaseL{$\boldsymbol{\round{f(a')}_{\Lambda} > 0 \lor \round{f(a')}_{\Lambda} < 0 \land \valv \in  (0, B)}$}
		%
		%
	let $\valv_1 = \valv - \frac{\Lambda}{2}$,
	%
	$\valv_2 = \valv + \frac{\Lambda}{2}$, we have $\valv_1, \valv_2 > 0$.
	%
	By the semantics and soundness theorem, the bounds derived for $\fvalL, \fvalR$ and $\fvalL', \fvalR'$
...
%
		...
		%
		$h(\fvalL') = e^{\epsilon 
				\big( (f(a') - \frac{\valv_1}{1 + \eta})) (1 + \eta)^2) \big)}$,
		%
		$g(\fvalL') = e^{\epsilon 
				\frac{(f(a') - \valv_1(1 + \eta)}{(1 + \eta)^2}}$.
		%
...
%
		...
		%		
		let $\valv_1 = \valv - \frac{\Lambda}{2}$, $\valv_2 = \valv - \frac{\Lambda}{2}$, we have $\valv_1, \valv_2 < 0$.
		%
		\\
		%
...
%
		...
		\\
		%
		By \textbf{Lemma \ref{lem:sign}, \ref{lem:clampid}, \ref{lem:roundrl}}, the semantics and soundness theorem, we derived following bounds for $\fvalL, \fvalR, \fvalL'$ and $\fvalR'$:
		%
		\\
		For input $a'$:
		%
