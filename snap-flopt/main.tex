...
\mbox{Real Expr.} & \rexpr & ::= & \rvarx ~|~ \rval
	%
	~|~ F(\data) ~|~ \rexpr \bop \rexpr
	%
	~|~ \uop (\rexpr) \\
%
\mbox{Floating Point Expr.} & \fexpr & ::= & \fvarx ~|~ \fval
	%
	~|~ f(\data) ~|~ \fexpr \obop \fexpr
	%
	~|~ \ouop (\fexpr) \\
%
...
%
\mbox{Evaluation Env.} & \evlenv & ::= & \cdot ~|~ \evlenv[x \mapsto \valv] \\

\end{array}
\]

$\obop$ and $\ouop$ represent the binary and unary operations in floating point computation.
%
$F(\data)$ and $f(\data)$ denotes primitive query $F$ evaluates to value $F(\data)$ and $f(\data)$ given input database $\data$ in real computation and floating point computation separately. We assume the query to be primitive, where the computation of the query results introduce no floating point error. We also assume the sample process, negation, rounding ($\round{}$) and clamping ($\clamp_B()$) are perfect, i.e., introducing no error in floating point computation.


\section{Semantics}

The transition semantics with relative floating point computation error are shown in Figure. \ref{fig_trans_semantics_exp}. The semantics are $\rexpr \bigstep \fexpr, \err$, which means a real expression $\rexpr$ can be transited in floating point computation expression $\fexpr$ with error bound $\err$, $\eta$ is the machine epsilon.
...
}
{
	\rval
	\trsto
	\fval, \big( r(1 + \eta), \frac{r}{(1 + \eta)} \big)
}
%
\and
%
...
\inferrule*[right = bop]
{
	\rexpr^1 \trsto \fexpr^1, (\ubar{\rexpr^1}, \bar{\rexpr^1})
	\and
	\rexpr^2 \trsto \fexpr^2, (\ubar{\rexpr^2}, \bar{\rexpr^2})
	\and
	\rexpr^1 * \rexpr^2 \geq 0
}
{
    \rexpr^1 * \rexpr^2
    \trsto \fexpr^1 \obop \fexpr^2,
    \big(
...
\inferrule*[right = uop]
{
	\rexpr \trsto \fexpr, (\ubar{\rexpr}, \bar{\rexpr})
	\and
	\uop (\rexpr) \geq 0
}
{
...
}
{
    ...
    \trsto \ouop(\fexpr),
    \big(
    (\uop(\ubar{\rexpr}))(1 + \eta),
    \frac{\uop(\bar{\rexpr})}{(1 + \eta)}
    \big)
}
\end{mathpar}
...
% \inferrule*[right = asg]
% {
% 	\env, \rexpr \bigstep \fexpr, \err 
% }
% {
% 	\env, \varx = \rexpr
% 	\bigstep
% 	\env[\varx \mapsto (\fexpr, \err )]
% }
% %
% \inferrule*[right = asg]
% {
...
{
	\fexpr^1 \bigstep \fval^1
	\and
	\fexpr^2 \bigstep \fval^2
	\and
	\fval^1 \obop \fval^2 = \fval
}
{
    \fexpr^1 \obop \fexpr^2 \bigstep \fval
}
%
\and
%
...
{
	\rexpr^1 \bigstep \rval^1
	\and
	\rexpr^2 \bigstep \rval^2
	\and
	\rval^1 \bop \rval^2 = \rval
}
{
    \rexpr^1 \bop \rexpr^2 \bigstep \rval
}
%
\and
%
...
{
    \uop(\rexpr) \bigstep \rval
}
\end{mathpar}
\caption{Semantics of Evaluation for Floating Point and Real Expressions}
\label{fig_real_semantics_exp}
\end{figure}


\begin{thm}[Soundness Theorem]
Given $\rexpr$ and $\fexpr$ where the transition 
$\rexpr \bigstep \fexpr, (\rexpr^1, \rexpr^2)$ holds, 
if $\fexpr$ evaluates to $c$ in floating point computation and 
$\rexpr$, $\rexpr^1$ and $\rexpr^2$ evaluates to $r$, $r^1$ and $r^2$ in real computation, then: 
\[
r^1 \leq c \leq r^2 
% ~~ \land ~~ 
...
\end{thm}
\begin{proof}
Induction on $\rexpr$, we have following cases:

\end{proof}

\newpage
\section{Semantics For Programs}

The transition semantics with relative floating point computation error are shown in Figure. \ref{fig_trans_semantics_exp} for programs. The semantics are $\trsenv, \rprog \bigstep \fprog, \trsenv$, which means a real programs $\rprog$ can be transited in floating point computation program $\fprog$ with error bound for all variables in $\trsenv$, $\eta$ is the machine epsilon.

\begin{figure}
\begin{mathpar}
\inferrule*[right = var]
{
	\trsenv(\rvarx) = ( \ubar{\rexpr}, \bar{\rexpr} )
}
{
	\trsenv, \rvarx
	\trsto
	( \ubar{\rexpr}, \bar{\rexpr} )
}
%
\and
%
...
\inferrule*[right = bop]
{
	\trsenv, \rexpr^1 \trsto (\ubar{\rexpr^1}, \bar{\rexpr^1})
	\and
	\trsenv, \rexpr^2 \trsto (\ubar{\rexpr^2}, \bar{\rexpr^2})
	\and
	\rexpr^1 * \rexpr^2 \geq 0
}
{
    \trsenv, \rexpr^1 * \rexpr^2
    \trsto
    \big(
    \frac{\ubar{\rexpr^1} * \ubar{\rexpr^2}}{(1 + \eta)}, 
    (\bar{\rexpr^1} * \bar{\rexpr^2})(1 + \eta)
...
\inferrule*[right = uop]
{
	\trsenv, \rexpr \trsto (\ubar{\rexpr}, \bar{\rexpr})
	\and
	\uop (\rexpr) \geq 0
}
{
...
\inferrule*[right = asg]
{
	\trsenv, \rexpr \trsto \err 
}
{
	\trsenv, \rvarx = \rexpr
	\trsto
	\trsenv[\rvarx \mapsto \err]
}
%
...
then $\forall x \in dom(\trsenv)$ s.t. $\trsenv(x) =(\expr, (\ubar{\rexpr}, \bar{\rexpr})) $, we have
if $\expr$ evaluates to $c$ in floating point computation and 
$\ubar{\rexpr}$ and $\bar{\rexpr}$ evaluates to $\ubar{r}$ and $\bar{r}$ in real computation, then: 
\[
...
Let $(l, r)$ be the range where $\forall u \in (l, r)$ and some $s$, $\fsnap'(a, u, s) = x$, we estimated the $|r - l|$ in terms of floating point relative error and $|R - L|$ through our semantics in order to verify the privacy loss of $\fsnap$.
	...
	\begin{itemize}
		...
		\end{mathpar}
		From soundness theorem, we have  $\rexpr^1 \leq y_2 \leq \rexpr^2$, where we can get $\ubar{R} \leq r \leq \bar{R}$.\\
		%
