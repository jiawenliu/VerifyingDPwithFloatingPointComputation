...
\begin{itemize}
	...
	We need to show $\trsenv_2$ is a safe environment.\\
	%
	Since we know $\trsenv$ is a safe environment by assumption $(\star)$, by induction hypothesis, we have:
	%
	\\
	$\trsenv_1$ and $\trsenv_2$ are all safe environment. This case is proved.
...
%
	...
	\inferrule*[right = bop]
		{
			\trsenv, \expr^1 \trsto (\ubar{\expr^1}, \bar{\expr^1}) ~ (\diamond)
			\and
			\trsenv, \expr^2 \trsto (\ubar{\expr^2}, \bar{\expr^2}) ~ (\triangle)
			\\
		\bar{\expr}, \ubar{\expr} = 
		\max, \min(\ubar{\expr^1} * \ubar{\expr^2}, 
		\bar{\expr^1} * \ubar{\expr^2}, 
