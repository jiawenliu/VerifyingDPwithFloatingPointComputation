...
% \begin{defn}
% [$\snap(a) : A \to \distr(\real)$]
% Given privacy parameter $\epsilon$, the fixed point implemented
...
\begin{thm}
[The $\snap$ mechanism is $(\epsilon 
+ \ln(\frac{e^2(e + 1)}{e^{1 - 4\eta \epsilon} + \eta}), 0)-$differentially private]
%
...
%
Since all the errors are accumulate on the same direction, 
		Without loss of generality, we prove the following case:
	%
	\begin{itemize}
		%
		%
