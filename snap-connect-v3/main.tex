...
}
{
	...
	: \top \Rightarrow 
	(\varx_2\langle 2 \rangle) 
	= (\varx_1\langle 1 \rangle )
	\land (\varx_2\langle 2 \rangle) = (\varx_1\langle 1 \rangle)
	\land (\varx_2\langle 2 \rangle) = (\varx_1\langle 1 \rangle)
}~\textbf{null}
\and
\inferrule
...
% {
% 	\vdash 
% 	\varx_1 = \clamp_B{(\vary_1)}	
% 	\sim_{0} 
% 	\varx_2 = \clamp_B{(\vary_2)}
% 	: (B \leq ( \vary_2 \langle 2 \rangle))
% 	\land 
% 	(B \leq ( \vary_2 \langle 2 \rangle))
% 	\Rightarrow 
% 	(\varx_2 \langle 2 \rangle) = (\varx_1 \langle 1 \rangle) = B
% }~\textbf{clamp}_r
% \and
% \inferrule
% {
...
\]
	...
	%
	\\
	\textbf{Subgoal:} For any pair of environments $(\trsmem_1, \trsmem_2)$, if we can show ${\muniform(0,1]} ~ R^{\#(\epsilon)} ~ {\muniform(0,1]}$, then $\mu^*_1(\trsmem_1) \sem{\Phi}^{\#(\epsilon)} \mu^*_2(\trsmem_2)$,
	%
	where
	\[
		R = 
	...
	\\
	%
	(i) For any $\muniform(0,1]\in \distr(\float)$, there is one and only one $\edistr^*_1(\trsmem_1) \in \distr(Env)$.
	%
	\\
	(ii) For any $\muniform(0,1] \in \distr(\float)$, there is one and only one $\edistr^*_2(\trsmem_2) \in \distr(Env)$.
	\\
	%
	...
	%
	%
	$\mu^*_1(\trsmem_1) \in \distr(\float_{\real \times \real})$ as 
	$\elet \fval_{1(\rval_{1l}, \rval_{1u})} = \sem{\uniform(0, 1)}_{\trsmem_1} 
	...
	\]
	%
	Then we have $\mu^*_1(\trsmem_1) \sem{\Phi}^{\#(\epsilon)} \mu^*_2(\trsmem_2)$ proved by \textbf{Subgoal}.
\caseL{}
	\caseL{[\textsc{Null}] }
	\caseL{[\textsc{Assn}] }
	\caseL{[\textsc{Seq}] }
...
Let $R' =
		\left \{(\fval_1, \fval_2) \in \float \times \float 
		~|~
		e^{-\epsilon} \fval_1 \leq \fval_2 \leq e^{\epsilon} \fval_1
		\right \}$, we know $R' \implies R$.
%
...
$\mu_L, \mu_R \in \distr(\float \times \float)$ 
...
% then we have:
% 	\[
% 	\sem{\varx_1 = f(a) + \frac{1}{\epsilon}\ln(\varu_1)}_{\trsmem_1}(\varx_1))
% 	\lameq 
% 	\sem{\varx_2 = f(a') + \frac{1}{\epsilon}\ln(\varu_2)}_{\trsmem_2}(\varx_2))	
% 	\]	
% \end{lem}
...
and $\rvalR_2 = e^{\epsilon 
		...
		\\
Assume $\rvalL_1 \leq u_1 \leq \rvalR_1$,
we have following derivation for this case:
...
\inferrule*[right = Seq]
{
	...
	}
	{
		...
		\top
		\Rightarrow 
		\rvalL_1 \leq u^1_1 \leq \rvalR_1 \implies 
		e^{\epsilon(1 + \eta)^2}\rvalL_1 \leq u^1_1 \leq e^{\epsilon(1 + \eta)^2}\rvalR_1
	}~\textbf{unif}
	~~
	\inferrule
	{
	...
	}
	{
		...
		: 
		\top \Rightarrow \Phi \land s_1\langle 1 \rangle) 
		= s_2\langle 2 \rangle)
	}
\\
}
{
	...
	\Rightarrow  \varx_1\langle 1 \rangle) 
	= \varx_2\langle 2 \rangle) = \valv
}
\\
...
\inferrule*[right = Assn]
	...
	}
	{
		...
		:
		\Phi \land s_1\langle 1 \rangle) = s_2\langle 2 \rangle)
		\Rightarrow
		\Phi'
	}
\and
\Phi' \Rightarrow \varx_1\langle 1 \rangle) \lameq \varx_2\langle 2 \rangle) 
}
{
	\varx_1 = f(a) + \frac{1}{\epsilon} \times s_1 \times \ln (\varu_1)
	\sim_{0} 
	...
	:
	f(a) + 1 = f(a') \land \Phi 
	\land s_1\langle 1 \rangle) = s_2\langle 2 \rangle)
	\Rightarrow \varx_1\langle 1 \rangle) \lameq \varx_2\langle 2 \rangle)
}
\and
\Delta_R
}
{
	...
	\varz_2 = \clamp_B(\vary_2)
	:
	f(a) + 1 = f(a') \land \Phi \land s_1\langle 1 \rangle) = s_2\langle 2 \rangle)
	\Rightarrow \varx_1\langle 1 \rangle) = \varx_2\langle 2 \rangle)
}
\\
\Delta_R:
\\
\inferrule*[right = Seq]
{
	...
	}
	{
		...
		\sim_{0}
		\vary_2 = \round{\varx_2}_{\Lambda}
		: \varx_1\langle 1 \rangle) \lameq \varx_2\langle 2 \rangle) 
		\Rightarrow \varx_1\langle 1 \rangle) = \varx_2\langle 2 \rangle)
	}
	\and
	\inferrule
	...
	}
	{
		...
		\sim_{0}
		\varz_2 = \clamp_B(\vary_2)
		: \varx_1\langle 1 \rangle) = \varx_2\langle 2 \rangle) 
		\Rightarrow \varx_1\langle 1 \rangle) = \varx_2\langle 2 \rangle)
	}~\textbf{null}
}
{
...
%
The $\Phi' \Rightarrow \varx_1) \lameq \varx_2)$ is proved as following:
%
\\
...
%
 We also have $\varu_1) = \varu_1) = \varu_1)$, 
	$\varu_2) = \varu_2) = \varu_2)$,
	$\varx_1) \leq \varx_1) \leq \varx_1)$ and 
	$\varx_2) \leq \varx_2) \leq \varx_2)$ by the semantics, 
	since $\Phi' = 	
\rvalL_1 \leq \varu_1) < \rvalR_1 
	\land 
	\rvalL_2 \leq \varu_2) < \rvalR_2$, 
	we can get:
{\tiny
\[
\begin{array}{rcl}
	...
	\\
	0 \leq \varx_1) \leq  b + \frac{\Lambda}{2}
	& ~ \sim ~ &
	0 \leq \varx_2) \leq b + \frac{\Lambda}{2}
	\\
	\varx_1) & ~ \lameq ~ & \varx_2)
\end{array}
\]
...
\inferrule*[right = Seq]
{
	...
	{
	\Phi \triangleq 
	\rvalL_1 \leq \varu_1 \langle 1 \rangle) \leq \rvalR_1 
	\land 
	\rvalL_2 \leq \varu_2 \langle 2 \rangle) \leq \rvalR_2
	}
	{
		\varu_1 \samplel \uniform(0,1)
