...
Theorem hoare_asgn_fwd :
  ...
  {{fun st => P (st & { X --> m })
            /\ st X = aeval  (st & { X --> m }) a }}.
Proof.
intros. unfold hoare_triple. intros. inversion H. subst. inversion H0. subst.
split.
- rewrite t_update_shadow. rewrite t_update_same. assumption.
- rewrite t_update_shadow. rewrite t_update_same. reflexivity.
...
exists (st X). split; try (rewrite t_update_shadow; rewrite t_update_same); 
...
Lemma silly1 : forall (P : nat -> nat -> Prop) (Q : nat -> Prop),
  ...
  Q 42.
Proof.
  intros P Q HP HQ. eapply HQ with ( y:= 0). apply HP.
...
Proof.
eapply hoare_seq with (Q := fun st => st X = 1).
- eapply hoare_consequence_pre with (P' := (fun st => st X = 1 /\ st Y = 2) [Y|-> 2]).
+ simpl. apply hoare_asgn.
+ unfold assn_sub, t_update. simpl. split.
{ assumption.  }
{ reflexivity. }
-  apply hoare_consequence_pre
    with (P' := (fun st => st X = 1) [X |-> 1]).
...
Theorem swap_exercise :
  ...
  {{fun st => st Y <= st X}}.
Proof.
unfold swap_program. apply hoare_seq with (Q := fun st: state => st Z <= st Y).
- apply hoare_seq with (Q := fun st: state => st Z <= st X).
+ apply hoare_consequence_pre
    with (P' := (fun st : state => st Y <= st X) [Y |-> Z]).
{ apply hoare_asgn. }
{ intros st H. unfold assn_sub, t_update. simpl. assumption. }
+ apply hoare_consequence_pre
    with (P' := (fun st : state => st Z <= st X) [X |-> Y]).
{ apply hoare_asgn. }
{ intros st H. unfold assn_sub, t_update. simpl. assumption. }
- apply hoare_consequence_pre
    with (P' := (fun st : state => st Z <= st Y) [Z |-> X]).
{ apply hoare_asgn. }
{ intros st H. unfold assn_sub, t_update. simpl. assumption. }
Qed.
...
rewrite le_plus_minus_r. reflexivity. assumption.
-  eapply hoare_consequence_pre. apply hoare_asgn.
    unfold assn_sub, t_update, assert_implies.
...
inversion HE; subst.
- apply (HTrue st st'). assumption. split; try assumption.
- apply (HFalse st' st'). apply E_Skip.  split; try assumption. apply bexp_eval_false. assumption.
Qed.
...
Proof.
  (* FILL IN HERE *)
unfold ex1_repeat.
 apply E_RepeatTrue with (st' := { --> 0} & {X --> 1; Y --> 1}).
- apply E_Seq with (st' := { --> 0} & {X --> 1}).
...
Proof.
  (* FILL IN HERE *) 
unfold hoare_triple. intros. unfold havoc_pre in H0.
