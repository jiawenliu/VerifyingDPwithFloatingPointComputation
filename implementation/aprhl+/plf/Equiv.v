...
+ apply E_IfFalse.
{ inversion H; subst; try (simpl; rewrite Hb; simpl; reflexivity).
}
{ inversion H; subst; try assumption. 
rewrite H5 in Hb. inversion Hb. 
}
+ apply E_IfTrue.
{ inversion H; subst; try (simpl; rewrite Hb; simpl; reflexivity).
}
...
- destruct (beval st (! b)) eqn:Hb.
{ inversion H. subst. 
{ simpl in Hb. apply negb_true_iff in Hb. apply E_IfFalse; try assumption. }
{ rewrite Hb in H5. inversion H5. }
}
{inversion H. subst. 
{ rewrite Hb in H5. inversion H5. }
{ simpl in Hb. apply negb_false_iff in Hb. apply E_IfTrue; try assumption. }
}
Qed.
...
Proof.
intros b c Hb. split; intros H.
- simpl in Hb. apply (WHILE_true_nonterm b c st st') in Hb. unfold not in Hb.
apply Hb in H. inversion H.
- simpl in H. assert (bequiv BTrue true). 
{ split. }
 apply (WHILE_true_nonterm BTrue SKIP st st') in H0. unfold not in H0.
...
Theorem assign_aequiv : forall (X:string) e,
  ...
  cequiv SKIP (X ::= e).
Proof.
intros. split; intros Hs.
-
replace st' with (st' & { X --> aeval st' e}).
inversion Hs; subst. apply E_Ass. reflexivity. 
unfold aequiv in H. rewrite <- H. simpl. rewrite t_update_same. reflexivity.
- inversion Hs; subst. unfold aequiv in H. rewrite <- H. simpl. 
...
:=[[prog_f;prog_g]; [prog_c;prog_h]; [prog_b; prog_e]; [prog_a]; [prog_i];[prog_d]].
...
Theorem CSeq_congruence : forall c1 c1' c2 c2',
  ...
  cequiv (c1;;c2) (c1';;c2').
Proof.
unfold cequiv. intros c1 c1' c2 c2' Hc1 Hc2. split; intros H.
- inversion H; subst. apply E_Seq with st'0. 
+ apply Hc1 in H2. assumption.
+ apply Hc2 in H5. assumption.
- inversion H; subst. apply E_Seq with st'0.
+ apply Hc1 in H2. assumption.
+ apply Hc2 in H5. assumption.
Qed.
...
- inversion H; subst.
+ apply E_IfTrue.
  * rewrite Hb in H5. assumption.
  * rewrite Hc1 in H6. assumption.
+ apply E_IfFalse.
  * rewrite Hb in H5. assumption.
  * rewrite Hc2 in H6. assumption.
- inversion H; subst.
+ apply E_IfTrue.
  * rewrite <- Hb in H5. assumption.
  * rewrite <- Hc1 in H6. assumption.
...
(** (Doing induction when there are a lot of constructors makes
  ...
  - (* BLe *)
    ...
    replace (aeval st a2) with (aeval st a2') by
       (subst a2'; rewrite <- fold_constants_aexp_sound; reflexivity).
    destruct a1'; destruct a2'; try reflexivity. simpl.
...
Proof.
  ...
  - (* WHILE *)
    ...
    + unfold cequiv. split. 
    { intros. apply WHILE_true_nonterm in H0. inversion H0. assumption. }
    { intros. apply WHILE_true_nonterm in H0. inversion H0. unfold bequiv. reflexivity. }
...
Fixpoint optimizer_com (c : com) : com := optimize_0plus_com (optimize_0plus_com c).
...
remember { --> 0 } as st.
remember { --> 0 } as st'.
assert ((WHILE true DO SKIP END) / st \\ st' <-> SKIP / st \\ st'). { apply (H st st'). }
assert ((SKIP / st \\ st')). { subst. apply E_Skip. }
apply H0 in H1.
apply WHILE_true_nonterm in H1.
assumption. simpl. split.
Qed.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_HAVOC : forall (st : state) (n : nat) (X : string), (HAVOC X) / st \\ (st & { X --> n})
...
Example havoc_example2 :
  (SKIP;; HAVOC Z) / { --> 0 } \\ { Z --> 42 }.
Proof.
intros. apply E_Seq with { --> 0 }. apply E_Skip. apply  E_HAVOC. Qed.
...
intros. left.
unfold cequiv.
split;  unfold pXY; unfold pYX; intros H.
- inversion H; subst. 
inversion H2; subst.
inversion H5; subst.
...
(** If you think they are equivalent, then prove it. If you think they
    ...
    useful.) *)
Lemma pcopy_eq :
 forall (n1 n2 : nat), pcopy /  { --> 0} \\ ( { X --> n1} &{ Y --> n2}) -> n1 = n2.
Proof.
intros.
inversion H. 
inversion H2.
inversion H5.
 assert (E1: aeval st' Imp.X = n).
{  rewrite <- H8. reflexivity. }
 rewrite H12 in E1.
rewrite <- H8 in H13.
assert (E2: {Imp.X --> n; Y --> n0} Imp.X = n). reflexivity.
rewrite H13 in E2.
assert (E3 : {Imp.X --> n1; Y --> n2} Imp.X = n1). reflexivity.
rewrite E2 in E3.
assert (E4 : {Imp.X --> n; Y --> n0} Y = n0). reflexivity.
rewrite H13 in E4.
assert (E5 : {Imp.X --> n1; Y --> n2} Y = n2). reflexivity.
rewrite E4 in E5.
rewrite <- E1 in E3.
rewrite E3 in E5.
assumption.
...
remember { --> 0 } as st.
remember { X --> 0 } as st'.
remember ({ X --> 0 } & { Y --> 1 }) as st''.
assert (ptwice / st \\ st'').
{ subst. unfold ptwice. apply E_Seq with {X --> 0}. 
apply E_HAVOC. apply E_HAVOC. }

assert (pcopy / st \\ st'').
{
apply (H st st'') in H0. assumption.
}
subst.
apply (pcopy_eq 0 1) in H1.
inversion H1.
Qed.
