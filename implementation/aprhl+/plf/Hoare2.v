...
Definition real_fact_dec (m : nat): decorated :=

    {{fun st => st X = m }} ->>
    {{ fun st =>          fact (st X) * 1 = fact m           }}
  Y ::= 1
    {{  fun st =>         fact (st X) * st Y =fact m           }};;
  WHILE !(X = 0)
  DO   {{fun st =>   fact (st X) * st Y = fact m /\ st X <> 0            }} ->>
       {{fun st =>   fact(st X - 1) * st X * st Y = fact m           }}
     Y ::= Y * X
       {{fun st =>    fact (st X - 1) * st Y = fact m              }};;
     X ::= X - 1
       {{fun st =>   fact (st X) * fact (st Y) = fact m                    }}
  END
       {{fun st =>    fact (st X) * fact (st Y) = fact m /\ st X = 0           }} ->>
    {{fun st =>   st Y = fact m }}
.
